...
const initBaseData = async () => {
    ...
    if (logisticsMethodCount === 0) {
        // 获取承运商
        const postCarrier = await Carrier_1.default.findOne({ where: { code: 'POST' } });
        const evriCarrier = await Carrier_1.default.findOne({ where: { code: 'EVRI' } });
        const yodelCarrier = await Carrier_1.default.findOne({ where: { code: 'YODEL' } });
        if (postCarrier && evriCarrier && yodelCarrier) {
            await LogisticsMethod_1.default.bulkCreate([
                {
                    name: 'Royal Mail',
                    code: 'RM',
...
// 导入报价数据
const importQuotations = async (workbook) => {
    // 获取基础数据
    const ukRegion = await Region_1.default.findOne({ where: { code: 'UK' } });
    if (!ukRegion) {
    ...
    // 清空现有报价数据
    await Quotation_1.default.destroy({ where: {} });
    ...
    for (const sheetName of priceSheetNames) {
        ...
        if (!logisticsMethod) {
            ...
            if (sheetName.includes('邮局')) {
                ...
                if (postCarrier) {
                    // 创建新的物流方式
                    logisticsMethod = await LogisticsMethod_1.default.create({
        ...
        if (quotationData.length > 0) {
            await Quotation_1.default.bulkCreate(quotationData);
...
// 执行数据导入
importQuotationData();
//# sourceMappingURL=importQuotationData.js.map