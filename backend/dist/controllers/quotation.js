...
const quotationController = {
    ...
    getAllQuotations: async (req, res) => {
        try {
            ...
            const where = {};
            // 获取总记录数
            const total = await Quotation_1.default.count({ where });
            // 获取分页数据，包含关联的重量范围
    ...
    searchQuotations: async (req, res) => {
        try {
            ...
            if (weight) {
                ...
                where.weight_to = { [sequelize_1.Op.gte]: weight };
            }
            if (volume) {
                where.volume_from = { [sequelize_1.Op.lte]: volume };
                where.volume_to = { [sequelize_1.Op.gte]: volume };
            }
            if (effective_date) {
                where.effective_date = { [sequelize_1.Op.lte]: effective_date };
                where.expire_date = { [sequelize_1.Op.or]: [{ [sequelize_1.Op.gte]: effective_date }, { [sequelize_1.Op.is]: null }] };
            }
            const quotations = await Quotation_1.default.findAll({
                where,
                include: [
    ...
    createQuotation: async (req, res) => {
        try {
            ...
            // 检查物流方式是否存在
            const logisticsMethod = await LogisticsMethod_1.default.findByPk(logistics_method_id);
            if (!logisticsMethod) {
            ...
            for (const range of weight_ranges) {
                ...
                await QuotationWeightRange_1.default.create(parsedRange);
            }
            // 创建报价附加费用
            if (additional_fees && Array.isArray(additional_fees)) {
                for (const fee of additional_fees) {
                    // 检查附加费用是否存在
                    const additionalFee = await AdditionalFee_1.default.findByPk(fee.additional_fee_id);
                    if (additionalFee) {
                        await QuotationAdditionalFee_1.default.create({
                            quotation_id: quotation.id,
            ...
            // 重新查询包含关联数据的报价
            const createdQuotation = await Quotation_1.default.findByPk(quotation.id, {
                include: [
                    { model: Carrier_1.default, attributes: ['id', 'name'], as: 'carrier' },
                    { model: LogisticsMethod_1.default, attributes: ['id', 'name'], as: 'logistics_method' },
                    { model: Region_1.default, attributes: ['id', 'name'], as: 'region' },
    ...
    updateQuotation: async (req, res) => {
        try {
            ...
            if (weight_ranges) {
                // 删除现有的重量范围
                await QuotationWeightRange_1.default.destroy({ where: { quotation_id: id } });
                // 创建新的重量范围
                for (const range of weight_ranges) {
                    await QuotationWeightRange_1.default.create({
                        quotation_id: quotation.id,
                        weight_from: parseFloat(range.weight_from),
                        weight_to: parseFloat(range.weight_to),
                        base_price: parseFloat(range.base_price),
            ...
            if (additional_fees && Array.isArray(additional_fees)) {
                // 删除现有的报价附加费用
                await QuotationAdditionalFee_1.default.destroy({ where: { quotation_id: id } });
                // 创建新的报价附加费用
                for (const fee of additional_fees) {
                    // 检查附加费用是否存在
                    const additionalFee = await AdditionalFee_1.default.findByPk(fee.additional_fee_id);
            ...
            // 重新查询包含关联数据的报价
            const updatedQuotation = await Quotation_1.default.findByPk(quotation.id, {
                include: [
                    { model: Carrier_1.default, attributes: ['id', 'name'], as: 'carrier' },
                    { model: LogisticsMethod_1.default, attributes: ['id', 'name'], as: 'logistics_method' },
                    { model: Region_1.default, attributes: ['id', 'name'], as: 'region' },
    ...
    // 导入报价
    importQuotations: async (req, res) => {
        try {
    ...
    // 导出报价
    exportQuotations: async (req, res) => {
        try {
